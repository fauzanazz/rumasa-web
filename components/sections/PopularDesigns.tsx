          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {signatureDesigns.map((design, idx) => (
              <div key={idx} className="bg-white rounded-xl shadow-lg overflow-hidden hover:shadow-xl transition-shadow">
                {/* House Illustration */}
                <div className={`${design.bgColor} h-64 flex items-center justify-center p-8`}>
                  <div className="relative w-48 h-32">
                    {/* Simple house shape */}
                    <div className="absolute inset-0 flex flex-col items-center justify-end">
                      {/* Roof */}
                      <div
                        className={`${design.roofColor} w-0 h-0`}
                        style={{
                      ...
                      />
                      {/* House body */}
                      <div className={`${design.houseColor} w-48 h-20 relative`}>
                        {/* Windows */}
                        <div className="absolute top-4 left-8 w-8 h-6 bg-blue-500"></div>
                        <div className="absolute top-4 right-8 w-8 h-6 bg-blue-500"></div>
                        {/* Door */}
                        {idx === 0 && (
                          <div className="absolute bottom-0 left-1/2 -translate-x-1/2 w-6 h-10 bg-blue-700"></div>
                        )}
                      </div>
                    </div>
                  </div>
